...
public class BlackScholes {
	...
	public static void main(String[] args) {
		...
		double	K=7900.0 ;  // Strike Price  
		double	S=7800.0 ;// Spot Price
		double	T=0.073972603; // E*piray days  in year   = ( option e*piration date - today's date) /365
		double	R=0.1 ; // Risk Free Rate ,constant value
		double	V=0.125310271; // Stock volatility
		double  dR=0.21; //Dividend rate   ( its needs to be set at the start of the month )
		...
		if (dR==0)
		{
			...
			//Case2: if we have dividend rate (dR1)
			//=K e^(-RT)(1-NORMSDIST (-D2)) - [S* (1-NORMSDIST (-D1)) * e^(-dRT)]
		    P=K*Math.exp(-R*T)*(1-Distribution.NORMSDIST(-D2))-(S* (1-Distribution.NORMSDIST(-D1))*Math.exp(-dR*T));
